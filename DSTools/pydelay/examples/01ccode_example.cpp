...
long unsigned int n_1b0fd9efa5279c4203b7c70233f86dbf;
...
int nstart = 101;
double dt0 = 0.01;
double maxdelay = 10.; // Set the maximum delay here !!!
...
double dt = dt0;
srand((unsigned)123);
n_1b0fd9efa5279c4203b7c70233f86dbf = SIM_n;
...
//due to the FSAL property k1(n+1)=k4(n)
	k1x = - pt_x_ar[SIM_n]  +  PARk *interp_x(t -10, &n_1b0fd9efa5279c4203b7c70233f86dbf)  +  PARA * f( PARw ,t);

while((t <= tfinal || hitdsc) && SIM_n-nstart <= MaxIter) {

	t += dt * 0.5;
	k2x = - (pt_x_ar[SIM_n] + 0.5 * dt * k1x)  +  PARk *interp_x(t -10, &n_1b0fd9efa5279c4203b7c70233f86dbf)  +  PARA * f( PARw ,t);
	t += dt * 0.25;
	k3x = - (pt_x_ar[SIM_n] + 0.75 * dt * k2x)  +  PARk *interp_x(t -10, &n_1b0fd9efa5279c4203b7c70233f86dbf)  +  PARA * f( PARw ,t);
	TEMPx = pt_x_ar[SIM_n] + dt * 1.0/9.0 * (2.0*k1x + 3.0*k2x + 4.0*k3x);
	t += dt * 0.25;
	k4x = - TEMPx  +  PARk *interp_x(t -10, &n_1b0fd9efa5279c4203b7c70233f86dbf)  +  PARA * f( PARw ,t);
...
// 49c2de40097f2152f650ce9055191cd2
