...
while((t <= tfinal || hitdsc) && SIM_n-nstart <= MaxIter) {
	...
	t += dt * 0.5;
	k2y1 = interp_y1(t - PARtau1, &plag2) ;
	k2y3 =  (pt_y2_ar[SIM_n] + 0.5 * dt * k1y2) ;
	k2y2 = interp_y1(t - PARtau1, &plag2) +interp_y2(t - PARtau2, &plag1) ;
	t += dt * 0.25;
	k3y1 = interp_y1(t - PARtau1, &plag2) ;
	k3y3 =  (pt_y2_ar[SIM_n] + 0.75 * dt * k2y2) ;
	k3y2 = interp_y1(t - PARtau1, &plag2) +interp_y2(t - PARtau2, &plag1) ;
