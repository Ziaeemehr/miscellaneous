...
long unsigned int lag1;
long unsigned int lag2;
...
//due to the FSAL property k1(n+1)=k4(n)
	k1y1 = interp_y5(t -1, &lag2) +interp_y3(t -1, &lag2) ;
	k1y3 = interp_y3(t -1, &lag2) +interp_y1(t -0.5, &lag1) ;
	k1y2 = interp_y1(t -1, &lag2) +interp_y2(t -0.5, &lag1) ;
	k1y5 = interp_y1(t -1, &lag2) ;
	k1y4 = interp_y5(t -1, &lag2) *interp_y4(t -1, &lag2) ;

while((t <= tfinal || hitdsc) && SIM_n-nstart <= MaxIter) {

	t += dt * 0.5;
	k2y1 = interp_y5(t -1, &lag2) +interp_y3(t -1, &lag2) ;
	k2y3 = interp_y3(t -1, &lag2) +interp_y1(t -0.5, &lag1) ;
	k2y2 = interp_y1(t -1, &lag2) +interp_y2(t -0.5, &lag1) ;
	k2y5 = interp_y1(t -1, &lag2) ;
	k2y4 = interp_y5(t -1, &lag2) *interp_y4(t -1, &lag2) ;
	t += dt * 0.25;
	k3y1 = interp_y5(t -1, &lag2) +interp_y3(t -1, &lag2) ;
	k3y3 = interp_y3(t -1, &lag2) +interp_y1(t -0.5, &lag1) ;
	k3y2 = interp_y1(t -1, &lag2) +interp_y2(t -0.5, &lag1) ;
	k3y5 = interp_y1(t -1, &lag2) ;
	k3y4 = interp_y5(t -1, &lag2) *interp_y4(t -1, &lag2) ;
