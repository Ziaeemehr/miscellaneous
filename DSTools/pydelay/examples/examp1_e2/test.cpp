...
//due to the FSAL property k1(n+1)=k4(n)
	...
	k1y3 =  pt_y2_ar[SIM_n] -sin(interp_y3(t - PARtau1, &plag1) );
...
while((t <= tfinal || hitdsc) && SIM_n-nstart <= MaxIter) {
	...
	k2y3 =  (pt_y2_ar[SIM_n] + 0.5 * dt * k1y2) -sin(interp_y3(t - PARtau1, &plag1) );
	k2y2 = interp_y1(t - PARtau1, &plag1) -interp_y2(t - PARtau2, &plag2) ;
	t += dt * 0.25;
	k3y1 = interp_y1(t - PARtau1, &plag1) ;
	k3y3 =  (pt_y2_ar[SIM_n] + 0.75 * dt * k2y2) -sin(interp_y3(t - PARtau1, &plag1) );
	...
	k4y3 =  TEMPy2 -sin(interp_y3(t - PARtau1, &plag1) );
