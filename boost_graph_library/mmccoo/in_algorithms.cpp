...
void example1()
{
    ...
    typedef typename graph_traits<MyGraphType>::vertex_descriptor vertex_descriptor;
    ...
    typedef property_map<MyGraphType, vertex_index_t>::type IdMap;
    ...
    iterator_property_map<std::vector<vertex_descriptor>::iterator,
                          ...
                          vertex_descriptor,
                          vertex_descriptor &>
        predmap(pred.begin(), get(vertex_index, G));
    ...
    iterator_property_map<int *,
                          ...
                          int,
                          int &>
        distmap_ptr(dist, get(vertex_index, G));
    ...
    iterator_property_map<std::vector<int>::iterator,
                          ...
                          int,
                          int &>
        distmap_vect(distvector.begin(), get(vertex_index, G));
...
public:
    ...
    my_weightmap(std::map<key_type, value_type> &weights) : weights(weights)
    { /* empty */
    }
...
void put(const my_distmap &dm, my_distmap::key_type k, my_distmap::value_type v)
{
    dm[k] = v;
}
...
public:
    ...
    my_predmap(std::map<key_type, value_type> &preds) : preds(preds)
    { /* empty */
    }
