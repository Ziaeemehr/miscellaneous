...
void example0a()
{
    ...
    // a simple graph with bundled properties for vertex and a predefined property for the edge.
...
void example1()
{
    ...
    typedef adjacency_list<vecS, vecS,
                           directedS,
                           VertexData,
                           property<edge_weight_t, double, EdgeData>>
        MyGraphType;
    MyGraphType G(5);
    G[0].first_name = "Jeremy";
...
void example2()
{
    ...
    typedef adjacency_list<vecS, vecS,
                           ...
                           property<vertex_distance_t, int,
                                    property<vertex_distance2_t, int>>,
                           property<edge_weight_t, double, EdgeData>>
        MyGraphType;
    MyGraphType G(5);

    property_map<MyGraphType, vertex_distance_t>::type distance_map =
        get(vertex_distance, G);
    property_map<MyGraphType, vertex_distance2_t>::type distance_map2 =
        get(vertex_distance2, G);

    auto v = add_vertex(G);
    distance_map[v] = 10;
    distance_map2[v] = 20;

    std::cout << "distance " << get(vertex_distance, G)[v] << std::endl;
    std::cout << "distance2 " << get(vertex_distance2, G)[v] << std::endl;
}
